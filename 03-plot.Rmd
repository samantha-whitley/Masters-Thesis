...
ggplot(df_sp_host, aes(x = year, y = ME_GDP, color = factor(dca))) +
  geom_line() +
...
#set critical threshold without canada
...
# Loop through each year in the results list
for(year in names(sar_results_contiguity)) {
  ...
  # Format effects and their p-values
...
# Remove the columns for p-values as they are now integrated with effects
combined_effects <- combined_effects %>%
...
combined_effects_table <- kable(combined_effects[, -1], format = "html", escape = FALSE, digits = 2,
      row.names = FALSE,
      col.names = c(col_names),
      caption = "Yearly Effects Breakdown") %>%
  kable_classic(full_width = F) %>%
  add_header_above(c("Defense burden (logged)" = 1, " " = 3)) %>%
  #footnote(general = "Standard errors are in brackets. * 10% Significance. ** 5% Significance. *** 1% Significance. \n Each value is the estimate of rho from a single-year from model 2.")  %>%
  pack_rows("2014", 1, 3) %>%
  pack_rows("2015", 4, 6) %>%
  pack_rows("2016", 7, 9) %>%
  pack_rows("2017", 10, 12) %>%
  pack_rows("2018", 13, 15) %>%
  pack_rows("2019", 16, 18) %>%
  pack_rows("2020", 19, 21) %>%
  pack_rows("2021", 22, 24) %>%
  pack_rows("2022", 25, 27) 
